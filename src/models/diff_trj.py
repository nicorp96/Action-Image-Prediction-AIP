...
from src.models.difussion_utils.utils_att import ParallelAttention
from src.models.difussion_utils.encoder import Encoder, find_traj_nn
...
class DiffusionHead(Encoder):
    ...
    def forward(
        ...
        rgb_feats_pyramid, pcd_pyramid = self.encode_images(visible_rgb, visible_pcd)
        ...
        for attn_round in range(self.attn_rounds):
            for scale in range(self.feat_scales):
                ...
                update = self._one_attention_round(
                    ...
                    instr_pos,  # language
                    curr_gripper_feats,
                    curr_gripper_pos,  # current gripper
                    goal_gripper_feats,
                    goal_gripper_pos,  # goal gripper
                    time_feats,
                    time_pos,  # time
                    traj_feats,
                    traj_pos,
                ...
                trajectory = torch.cat(
                    (trajectory[..., :3] + update[..., :3], update[..., 3:]), -1
                )
    ...
    def _one_attention_round(
        ...
        context_feats = einops.rearrange(
            rgb_feats_pyramid[scale], "b ncam c h w -> b (ncam h w) c"
        )
        context_pos = pcd_pyramid[scale]
        if p_inds is not None:
            context_feats = torch.stack(
                [f[i] for f, i in zip(context_feats, p_inds)]  # (nn, c)
            )
            context_pos = torch.stack([f[i] for f, i in zip(context_pos, p_inds)])
        ...
        if self.use_instruction:
            traj_feats, _ = self.traj_lang_attention[l_offset](
                seq1=traj_feats,
                seq1_key_padding_mask=trajectory_mask,
                seq2=instr_feats,
                seq2_key_padding_mask=None,
                seq1_pos=None,
                seq2_pos=None,

...
class LinearDiTActionSeqISim(DiTActionSeq):
    # https://github.com/bytedance/IRASim/blob/main/models/irasim.py
    def __init__(self, config):
        super().__init__(config)
        self.temp_embed = nn.Parameter(
...
class DiTActionSeqISimMultiCondi(DiTActionSeq):
    ...
    def __init__(self, config):
        ...
        self.temp_embed.data.copy_(torch.from_numpy(temp_embed).float().unsqueeze(0))
        self.condition_emb = ConditionEmbedding(out_dim=96 * 10)
...
class LinearDiTActionMultiCondi(LinearDiTActionSeqISim):
    # https://arxiv.org/pdf/2302.05543
    def __init__(self, config):
        super().__init__(config)
        self.temp_embed = nn.Parameter(
...
class DiTActionSeqJoint(DiTActionSeq):
    # https://arxiv.org/pdf/2302.05543
    def __init__(self, config):
        super().__init__(config)
        self.temp_embed = nn.Parameter(
        ...
        self.temp_embed.data.copy_(torch.from_numpy(temp_embed).float().unsqueeze(0))
        self.first_block = MMDiTBlockJointAction(
            self.hidden_size,
            self.num_heads,
            mlp_ratio=self.mlp_ratio,
        )

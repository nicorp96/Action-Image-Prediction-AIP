# Taken from: https://github.com/facebookresearch/DiT/blob/main/diffusion/gaussian_diffusion.py
...
class GaussianDiffusionSeq:
    ...
    def _prior_bpd(self, x_start):
        ...
        kl_prior = gd.normal_kl(
            mean1=qt_mean, logvar1=qt_log_variance, mean2=0.0, logvar2=0.0
        )
        return gd.mean_flat(kl_prior) / np.log(2.0)
    ...
    def calc_bpd_loop(self, model, x_start, clip_denoised=True, model_kwargs=None):
        ...
        for t in list(range(self.num_timesteps))[::-1]:
            ...
            xstart_mse.append(gd.mean_flat((out["pred_xstart"] - x_start) ** 2))
            eps = self._predict_eps_from_xstart(x_t, t_batch, out["pred_xstart"])
            mse.append(gd.mean_flat((eps - noise) ** 2))
...
class GaussianDiffusionSeqActAtt(GaussianDiffusionSeq):
    def __init__(self, *, betas, model_mean_type, model_var_type, loss_type):
        super().__init__(
            betas=betas,
            model_mean_type=model_mean_type,
        ...
        self.att = nn.MultiheadAttention(embed_dim=256, num_heads=4, device="cuda:0")
    ...
    def p_sample(
        ...
        noise = torch.randn_like(x)
        bz, sl, ch, h, w = noise.shape
        noise = rearrange(noise, "b l c h w -> b l (c h w)", l=sl)
        noise = rearrange(noise, "b l f -> l b f")
        noise, _ = self.att(noise, noise, noise)
        noise = rearrange(noise, "l b f -> b l f")
        noise = rearrange(noise, "b l (c h w) -> b l c h w", h=h, w=w, c=ch)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        unmask_x_t = self.q_sample(unmask_x_start, t, noise=unmask_noise)
        x_t = torch.cat([mask_x_x_start, unmask_x_t], dim=1)
        bz, sl, ch, h, w = x_t.shape
        x_t = rearrange(x_t, "b l c h w -> b l (c h w)", l=sl)
        x_t = rearrange(x_t, "b l f -> l b f")
        x_t, _ = self.att(x_t, x_t, x_t)
        x_t = rearrange(x_t, "l b f -> b l f")
        x_t = rearrange(x_t, "b l (c h w) -> b l c h w", h=h, w=w, c=ch)
        a_t = x_action[
            :, :mask_frame_num, :
        ]  # self.q_sample(x_action, t, noise=action_noised)
        model_kwargs["a"] = a_t
